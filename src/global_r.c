...
/*-<a                             href  ="qh-globa_r.htm#TOC"
  ...
  CMakeLists.txt check that CmakeLists @version is the same as qh_version2
    change year:    Copying.txt
    check download size
    recompile user_eg_r.c, rbox_r.c, libqhull_r.c, qconvex_r.c, qdelaun_r.c
  qvoronoi_r.c, qhalf_r.c, testqset_r.c
*/
...
void qh_checkflags(qhT* qh, char* command, char* hiddenflags) {
  ...
  if(*hiddenflags != ' ' || hiddenflags[strlen(hiddenflags) - 1] != ' ') {
    qh_fprintf(qh, qh->ferr, 6026,
               "qhull error (qh_checkflags): hiddenflags must start and end "
               "with a space: \"%s\"",
               hiddenflags);
    qh_errexit(qh, qh_ERRinput, NULL, NULL);
  }
  if(strpbrk(hiddenflags, ",\n\r\t")) {
    qh_fprintf(qh, qh->ferr, 6027,
               "qhull error (qh_checkflags): hiddenflags contains commas, "
               "newlines, or tabs: \"%s\"",
               hiddenflags);
    qh_errexit(qh, qh_ERRinput, NULL, NULL);
  }
...
unsigned long qh_clock(qhT* qh) {
  ...
  if(times(&time) == -1) {
    qh_fprintf(qh, qh->ferr, 6031,
               "qhull internal error (qh_clock): times() failed.  Use "
               "qh_CLOCKtype 1 in user.h\n");
    qh_errexit(qh, qh_ERRqhull, NULL, NULL);
  }
...
void qh_freebuffers(qhT* qh) {
  ...
  qh_memfree(qh, qh->gm_matrix,
             (qh->hull_dim + 1) * qh->hull_dim * sizeof(coordT));
...
void qh_init_qhull_command(qhT* qh, int argc, char* argv[]) {
  if(!qh_argv_to_command(argc, argv, qh->qhull_command,
...
void qh_initflags(qhT* qh, char* command) {
  ...
  if(qh->NOerrexit) {
    qh_fprintf(
        qh, qh->ferr, 6245,
        "qhull initflags error: qh.NOerrexit was not cleared before calling "
        "qh_initflags().  It should be cleared after setjmp().  Exit qhull.");
    qh_exit(6245);
  }
  ...
  while(*s) {
    ...
    switch(*s++) {
      ...
      case 'E':
        if(*s == '-')
          qh_fprintf(qh, qh->ferr, 7004,
                     "qhull warning: negative maximum roundoff given for "
                     "option 'An'.  Ignored.\n");
        else if(!isdigit(*s))
          qh_fprintf(qh, qh->ferr, 7005,
                     "qhull warning: no maximum roundoff given for option "
                     "'En'.  Ignored.\n");
        else {
        ...
        while(t > s) {
          if(*t && !isspace(*t)) {
            if(*t == ',')
              ...
              qh_fprintf(qh, qh->ferr, 7006,
                         "qhull warning: origin for Halfspace intersection "
                         "should be 'Hn,n,n,...'\n");
          }
          s = t;
          qh_strtod(s, &t);
      ...
      case 'R':
        if(!isdigit(*s))
          qh_fprintf(qh, qh->ferr, 7007,
                     "qhull warning: missing random perturbation for option "
                     "'Rn'.  Ignored\n");
        else {
      ...
      case 'U':
        if(!isdigit(*s) && *s != '-')
          qh_fprintf(qh, qh->ferr, 7009,
                     "qhull warning: missing coplanar distance for option "
                     "'Un'.  Ignored\n");
        else {
          ...
          switch(*s++) {
            ...
            case 'D':
              if(!isdigit(*s))
                ...
                if(qh->DROPdim >= 0)
                  qh_fprintf(qh, qh->ferr, 7013,
                             "qhull warning: can only drop one dimension.  "
                             "Previous 'GD%d' ignored\n",
                             qh->DROPdim);
      ...
      case 'P':
        while(*s && !isspace(*s)) {
          switch(*s++) {
            ...
            case 'A':
              if(!isdigit(*s))
                qh_fprintf(qh, qh->ferr, 6036,
                           "qhull input error: missing facet count for keep "
                           "area option 'PAn'\n");
              else {
        ...
        while(*s && !isspace(*s)) {
          switch(*s++) {
            ...
            case 'T':
              ...
              if(!isdigit(*s))
                qh_fprintf(qh, qh->ferr, 6039,
                           "qhull input error: missing number of test points "
                           "for option 'QTn'\n");
              else {
            ...
            case '1':
              ...
              switch(*s++) {
                ...
                default:
                  s--;
                  qh_fprintf(qh, qh->ferr, 7016,
                             "qhull warning: unknown 'Q' qhull option 1%c, "
                             "rest ignored\n",
                             (int)s[0]);
                  while(*++s && !isspace(*s))
                    ;
            ...
            LABELcheckdigit:
              if(isdigit(*s))
                qh_fprintf(qh, qh->ferr, 7017,
                           "qhull warning: can not follow '1', '2', or '3' "
                           "with a digit.  '%c' skipped.\n",
                           *s++);
              break;
            case '4':
            ...
            case 'G':
              i = qh_strtol(s, &t);
              if(qh->GOODpoint)
                qh_fprintf(qh, qh->ferr, 7018,
                           "qhull warning: good point already defined for "
                           "option 'QGn'.  Ignored\n");
              else if(s == t)
                qh_fprintf(qh, qh->ferr, 7019,
                           "qhull warning: missing good point id for option "
                           "'QGn'.  Ignored\n");
              else if(i < 0 || *s == '-') {
                qh->GOODpoint = i - 1;
            ...
            case 'R':
              if(!isdigit(*s) && *s != '-')
                qh_fprintf(qh, qh->ferr, 7020,
                           "qhull warning: missing random seed for option "
                           "'QRn'.  Ignored\n");
              else {
            ...
            case 'V':
              i = qh_strtol(s, &t);
              if(qh->GOODvertex)
                qh_fprintf(qh, qh->ferr, 7021,
                           "qhull warning: good vertex already defined for "
                           "option 'QVn'.  Ignored\n");
              else if(s == t)
                qh_fprintf(qh, qh->ferr, 7022,
                           "qhull warning: no good point id given for option "
                           "'QVn'.  Ignored\n");
              else if(i < 0) {
                qh->GOODvertex = i - 1;
            ...
            switch(*s++) {
              ...
              case 'z':
                if(qh->ferr == qh_FILEstderr) {
                  ...
                  qh_fprintf(qh, qh->ferr, 7024,
                             "qhull warning: output file undefined(stdout).  "
                             "Option 'Tz' ignored.\n");
                else {
                  qh_option(qh, "Tz-stdout", NULL, NULL);
                  qh->USEstdout = True;
              ...
              case 'C':
                if(!isdigit(*s))
                  qh_fprintf(qh, qh->ferr, 7025,
                             "qhull warning: missing point id for cone for "
                             "trace option 'TCn'.  Ignored\n");
                else {
                  i = qh_strtol(s, &s);
              ...
              case 'F':
                if(!isdigit(*s))
                  qh_fprintf(qh, qh->ferr, 7026,
                             "qhull warning: missing frequency count for trace "
                             "option 'TFn'.  Ignored\n");
                else {
              ...
              case 'I':
                if(!isspace(*s))
                  qh_fprintf(qh, qh->ferr, 7027,
                             "qhull warning: missing space between 'TI' and "
                             "filename, %s\n",
                             s);
                while(isspace(*s))
                  s++;
              ...
              case 'O':
                if(!isspace(*s))
                  qh_fprintf(qh, qh->ferr, 7028,
                             "qhull warning: missing space between 'TO' and "
                             "filename, %s\n",
                             s);
                while(isspace(*s))
                  s++;
                t = qh_skipfilename(qh, s);
                {
                  ...
                  s = t;
                  if(!qh->fout) {
                    qh_fprintf(qh, qh->ferr, 6266,
                               "qhull input warning: qh.fout was not set by "
                               "caller.  Cannot use option 'TO' to redirect "
                               "output.  Ignoring option 'TO'\n");
                  } else if(!freopen(filename, "w", qh->fout)) {
                    qh_fprintf(qh, qh->ferr, 6044,
                               "qhull error: could not open file \"%s\".",
                               filename);
                    qh_errexit(qh, qh_ERRinput, NULL, NULL);
                  } else {
              ...
              case 'P':
                if(!isdigit(*s))
                  qh_fprintf(qh, qh->ferr, 7029,
                             "qhull warning: missing point id for trace option "
                             "'TPn'.  Ignored\n");
                else {
              ...
              case 'M':
                if(!isdigit(*s))
                  qh_fprintf(qh, qh->ferr, 7030,
                             "qhull warning: missing merge id for trace option "
                             "'TMn'.  Ignored\n");
                else {
              ...
              case 'R':
                if(!isdigit(*s))
                  qh_fprintf(qh, qh->ferr, 7031,
                             "qhull warning: missing rerun count for trace "
                             "option 'TRn'.  Ignored\n");
                else {
              ...
              case 'V':
                i = qh_strtol(s, &t);
                if(s == t)
                  qh_fprintf(qh, qh->ferr, 7032,
                             "qhull warning: missing furthest point id for "
                             "trace option 'TVn'.  Ignored\n");
                else if(i < 0) {
                  qh->STOPpoint = i - 1;
              ...
              case 'W':
                if(!isdigit(*s))
                  qh_fprintf(qh, qh->ferr, 7033,
                             "qhull warning: missing max width for trace "
                             "option 'TWn'.  Ignored\n");
                else {
  ...
  if(qh->STOPcone && qh->JOGGLEmax < REALmax / 2)
    qh_fprintf(qh, qh->ferr, 7078,
               "qhull warning: 'TCn' (stopCone) ignored when used with 'QJn' "
               "(joggle)\n");
  if(isgeom && !qh->FORCEoutput && qh->PRINTout[1])
    qh_fprintf(qh, qh->ferr, 7037,
               "qhull warning: additional output formats are not compatible "
               "with Geomview\n");
...
void qh_initqhull_buffers(qhT* qh) {
  ...
  qh->lower_threshold =
      (realT*)qh_memalloc(qh, (qh->input_dim + 1) * sizeof(realT));
  qh->upper_threshold =
      (realT*)qh_memalloc(qh, (qh->input_dim + 1) * sizeof(realT));
  qh->lower_bound =
      (realT*)qh_memalloc(qh, (qh->input_dim + 1) * sizeof(realT));
  ...
  qh->gm_matrix = (coordT*)qh_memalloc(
      qh, (qh->hull_dim + 1) * qh->hull_dim * sizeof(coordT));
...
*/
void qh_initqhull_globals(qhT* qh,
                          coordT* points,
                          int numpoints,
                          int dim,
                          boolT ismalloc) {
...
#ifdef qh_NOmerge
  if(qh->MERGING) {
    qh_fprintf(qh, qh->ferr, 6045,
               "qhull input error: merging not installed(qh_NOmerge + 'Qx', "
               "'Cn' or 'An')\n");
    qh_errexit(qh, qh_ERRinput, NULL, NULL);
  }
#endif
  ...
  if(qh->DELAUNAY && qh->HALFspace) {
    qh_fprintf(qh, qh->ferr, 6046,
               "qhull input error: can not use Delaunay('d') or Voronoi('v') "
               "with halfspace intersection('H')\n");
    qh_errexit(qh, qh_ERRinput, NULL, NULL);
  }
  if(!qh->DELAUNAY && (qh->UPPERdelaunay || qh->ATinfinity)) {
    qh_fprintf(qh, qh->ferr, 6047,
               "qhull input error: use upper-Delaunay('Qu') or "
               "infinity-point('Qz') with Delaunay('d') or Voronoi('v')\n");
    qh_errexit(qh, qh_ERRinput, NULL, NULL);
  }
  if(qh->UPPERdelaunay && qh->ATinfinity) {
    qh_fprintf(qh, qh->ferr, 6048,
               "qhull input error: can not use infinity-point('Qz') with "
               "upper-Delaunay('Qu')\n");
    qh_errexit(qh, qh_ERRinput, NULL, NULL);
  }
  if(qh->SCALElast && !qh->DELAUNAY && qh->PRINTprecision)
    qh_fprintf(qh, qh->ferr, 7040,
               "qhull input warning: option 'Qbb' (scale-last-coordinate) is "
               "normally used with 'd' or 'v'\n");
  qh->DOcheckmax = (!qh->SKIPcheckmax && qh->MERGING);
  qh->KEEPnearinside =
      (qh->DOcheckmax && !(qh->KEEPinside && qh->KEEPcoplanar) &&
       !qh->NOnearinside);
  if(qh->MERGING)
...
/*-<a                             href="qh-globa_r.htm#TOC"
  ...
  qh_initqhull_mem(qh, )
    ...
    if qh.MERGING,
      includes ridgeT
    calls qh_user_memsizes(qh) to add up to 10 additional sizes for quick
...
void qh_initqhull_mem(qhT* qh) {
  ...
  numsizes = 8 + 10;
  qh_meminitbuffers(qh, qh->IStracing, qh_MEMalign, numsizes, qh_MEMbufsize,
                    qh_MEMinitbuf);
...
void qh_initqhull_outputflags(qhT* qh) {
  ...
  if(!(qh->PRINTgood || qh->PRINTneighbors)) {
    if(qh->KEEParea || qh->KEEPminArea < REALmax / 2 || qh->KEEPmerge ||
       qh->DELAUNAY || (!qh->ONLYgood && (qh->GOODvertex || qh->GOODpoint))) {
      qh->PRINTgood = True;
  ...
  if(printcoplanar && qh->DELAUNAY && qh->JOGGLEmax < REALmax / 2) {
    if(qh->PRINTprecision)
      qh_fprintf(qh, qh->ferr, 7041,
                 "qhull input warning: 'QJ' (joggle) will usually prevent "
                 "coincident input sites for options 'Fc' and 'FP'\n");
  }
  if(printmath && (qh->hull_dim > 3 || qh->VORONOI)) {
    qh_fprintf(
        qh, qh->ferr, 6056,
        "qhull input error: Mathematica and Maple output is only available for "
        "2-d and 3-d convex hulls and 2-d Delaunay triangulations\n");
    qh_errexit(qh, qh_ERRinput, NULL, NULL);
  }
  if(printgeom) {
    if(qh->hull_dim > 4) {
      qh_fprintf(qh, qh->ferr, 6057,
                 "qhull input error: Geomview output is only available for "
                 "2-d, 3-d and 4-d\n");
      qh_errexit(qh, qh_ERRinput, NULL, NULL);
    }
    if(qh->PRINTnoplanes &&
       !(qh->PRINTcoplanar + qh->PRINTcentrums + qh->PRINTdots +
    ...
    if(qh->VORONOI && (qh->hull_dim > 3 || qh->DROPdim >= 0)) {
      qh_fprintf(qh, qh->ferr, 6059,
                 "qhull input error: Geomview output for Voronoi diagrams only "
                 "for 2-d\n");
      qh_errexit(qh, qh_ERRinput, NULL, NULL);
    }
...
available for 4-d output(ignored).  Could use 'GDn' instead.\n");
  ...
  if(!qh->KEEPcoplanar && !qh->KEEPinside && !qh->ONLYgood) {
    if((qh->PRINTcoplanar && qh->PRINTspheres) || printcoplanar) {
      if(qh->QHULLfinished) {
        qh_fprintf(qh, qh->ferr, 7072,
                   "qhull output warning: ignoring coplanar points, option "
                   "'Qc' was not set for the first run of qhull.\n");
      } else {
        qh->KEEPcoplanar = True;
  ...
  if(qh->DROPdim >= 0) { /* after Geomview checks */
    if(qh->DROPdim < qh->hull_dim) {
      ...
      if(!printgeom || qh->hull_dim < 3)
        qh_fprintf(qh, qh->ferr, 7043,
                   "qhull input warning: drop dimension 'GD%d' is only "
                   "available for 3-d/4-d Geomview\n",
                   qh->DROPdim);
    } else
      qh->DROPdim = -1;
  } else if(qh->VORONOI) {
...
/*-<a                             href="qh-globa_r.htm#TOC"
  ...
  notes:
    report errors elsewhere, error handling and g_qhull_output [Qhull.cpp,
  QhullQh()] not in initialized see: qh_maxmin() determines the precision
  constants qh_freeqhull()
*/
void qh_initqhull_start2(qhT* qh, FILE* infile, FILE* outfile, FILE* errfile) {
  ...
  /* memset is the same in qh_freeqhull() and qh_initqhull_start2() */
  memset((char*)qh, 0,
         sizeof(qhT) - sizeof(qhmemT) -
             sizeof(qhstatT)); /* every field is 0, FALSE, NULL */
  qh->NOerrexit = True;
  qh->ANGLEmerge = True;
  qh->DROPdim = -1;
  qh->ferr = errfile;
  ...
  while(*s) {
    ...
    if(*s == 'P') {
      ...
      while(*s && !isspace(key = *s++)) {
        if(key == 'd' || key == 'D') {
          if(!isdigit(*s)) {
            qh_fprintf(qh, qh->ferr, 7044,
                       "qhull warning: no dimension given for Print option "
                       "'%c' at: %s.  Ignored\n",
                       key, s - 1);
            continue;
          }
          ...
          if(*s == ':') {
            ...
            if(fabs((double)value) > 1.0) {
              qh_fprintf(qh, qh->ferr, 7046,
                         "qhull warning: value %2.4g for Print option %c is > "
                         "+1 or < -1.  Ignored\n",
                         value, key);
              continue;
            }
      ...
      while(*s && !isspace(key = *s++)) {
        if(key == 'b' && *s == 'B') {
          ...
          idx = qh_strtol(s, &s);
          if(idx >= maxdim) {
            qh_fprintf(qh, qh->ferr, 7048,
                       "qhull warning: dimension %d for Qhull option %c is >= "
                       "%d.  Ignored\n",
                       idx, key, maxdim);
            continue;
          }
...
/*-<a                             href="qh-globa_r.htm#TOC"
  ...
  qh_lib_check( qhullLibraryType, qhTsize, vertexTsize, ridgeTsize, facetTsize,
  setTsize, qhmemTsize ) Report error if library does not agree with caller
...
void qh_lib_check(int qhullLibraryType,
  ...
  boolT iserror = False;
                                                                   ...
                                                                   */
  // _CrtSetBreakAlloc(744);  /* Break at memalloc {744}, or 'watch'
  // _crtBreakAlloc */
  _CrtSetDbgFlag(_CRTDBG_ALLOC_MEM_DF | _CRTDBG_DELAY_FREE_MEM_DF |
                 _CRTDBG_LEAK_CHECK_DF | _CrtSetDbgFlag(_CRTDBG_REPORT_FLAG));
  ...
  if(qhullLibraryType == QHULL_NON_REENTRANT) { /* 0 */
    qh_fprintf_stderr(
        6257,
        "qh_lib_check: Incorrect qhull library called.  Caller uses "
        "non-reentrant Qhull with a static qhT.  Library is reentrant.\n");
    iserror = True;
  } else if(qhullLibraryType == QHULL_QH_POINTER) { /* 1 */
    qh_fprintf_stderr(6258,
                      "qh_lib_check: Incorrect qhull library called.  Caller "
                      "uses non-reentrant Qhull with a dynamic qhT via "
                      "qh_QHpointer.  Library is reentrant.\n");
    iserror = True;
  } else if(qhullLibraryType != QHULL_REENTRANT) { /* 2 */
    qh_fprintf_stderr(
        6262,
        "qh_lib_check: Expecting qhullLibraryType QHULL_NON_REENTRANT(0), "
        "QHULL_QH_POINTER(1), or QHULL_REENTRANT(2).  Got %d\n",
        qhullLibraryType);
    iserror = True;
  }
  if(qhTsize != sizeof(qhT)) {
    qh_fprintf_stderr(6249,
                      "qh_lib_check: Incorrect qhull library called.  Size of "
                      "qhT for caller is %d, but for library is %d.\n",
                      qhTsize, sizeof(qhT));
    iserror = True;
  }
  if(vertexTsize != sizeof(vertexT)) {
    qh_fprintf_stderr(6250,
                      "qh_lib_check: Incorrect qhull library called.  Size of "
                      "vertexT for caller is %d, but for library is %d.\n",
                      vertexTsize, sizeof(vertexT));
    iserror = True;
  }
  if(ridgeTsize != sizeof(ridgeT)) {
    qh_fprintf_stderr(6251,
                      "qh_lib_check: Incorrect qhull library called.  Size of "
                      "ridgeT for caller is %d, but for library is %d.\n",
                      ridgeTsize, sizeof(ridgeT));
    iserror = True;
  }
  if(facetTsize != sizeof(facetT)) {
    qh_fprintf_stderr(6252,
                      "qh_lib_check: Incorrect qhull library called.  Size of "
                      "facetT for caller is %d, but for library is %d.\n",
                      facetTsize, sizeof(facetT));
    iserror = True;
  }
  if(setTsize && setTsize != sizeof(setT)) {
    qh_fprintf_stderr(6253,
                      "qh_lib_check: Incorrect qhull library called.  Size of "
                      "setT for caller is %d, but for library is %d.\n",
                      setTsize, sizeof(setT));
    iserror = True;
  }
  if(qhmemTsize && qhmemTsize != sizeof(qhmemT)) {
    qh_fprintf_stderr(6254,
                      "qh_lib_check: Incorrect qhull library called.  Size of "
                      "qhmemT for caller is %d, but for library is %d.\n",
                      qhmemTsize, sizeof(qhmemT));
    iserror = True;
  }
  if(iserror) {
    qh_fprintf_stderr(6259,
                      "qh_lib_check: Cannot continue.  Library '%s' is "
                      "reentrant (e.g., qhull_r.so)\n",
                      qh_version2);
